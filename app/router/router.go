...
import (
	...
	"adm/app/controller"
	admin "adm/app/controller/admin"
	mw "adm/app/router/middleware"
...
func HandleRoutes() *chi.Mux {
	...
	r.Route("/", func(r chi.Router) {
		...
		// home signup
		r.Get("/", h(controller.Home))

		// contact
		r.Get("/contact/new", h(controller.NewSupportMessage))
		r.Post("/contact", h(controller.CreateSupportMessage))
		r.Get("/pages/{slug}", h(controller.ShowPage))

		// Sitemap
		r.Get("/sitemap.xml", h(controller.RenderSitemap))
...
func h(fn controller.Handler) http.HandlerFunc {
	return controller.Handler(fn).ServeHTTP
}
