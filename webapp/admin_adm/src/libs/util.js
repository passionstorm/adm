...
export const getBreadCrumbList = (route, homeRoute) => {
  ...
  if (routeMetched.some(
    item => item.name === homeRoute.name)) return [homeItem];
...
export const showTitle = (item, vm) => {
  ...
  if (useI18n) {
    if (title.includes('{{') && title.includes('}}') &&
      useI18n) title = title.replace(/({{[\s\S]+?}})/,
...
export const collectIndexedRoute = (routers, list, parent = '') => {
  routers.forEach(r => {
    if (r.children && r.children.length) {
      collectRoute(r.children, list, r.name);
    }
    list[r.name] = {
...
const hasAccess = (access, route) => {
  if (route.meta && route.meta.access) return hasOneOf(access,
    route.meta.access);
  else return true;
};
